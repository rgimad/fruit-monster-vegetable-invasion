...
class Bonus:
    ...
    def bonus_type(self, bonus):
        ...
        return [self.game.player.health, self.game.player.bullets_num,
                self.game.player.bullet_speed, self.game.player.player_speed, self.game.player.bullet_size, self.game.player.reload_speed]
...
class MenuTestCase(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual([self.menu1.points_in_first_menu[0][2], self.menu1.points_in_first_menu[1][2], self.menu1.points_in_first_menu[2][2]], [u"Играть", u"Управление", u"Выйти"])
...
class GameTestCase(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual((self.game1.SCREEN_WIDTH, self.game1.SCREEN_HEIGHT), (1600, 900))

class CameraTestCase(unittest.TestCase):
    def setUp(self):
        self.map1 = Map()
        self.map1.load_from('assets/maps/map4.txt')
        self.game1 = Game()
        self.player1 = Player(self.game1, self.game1.all_sprites)
        self.camera1 = Camera(self.player1.rect.x, self.player1.rect.y)
        self.player1.rect.move_ip(0, 600)
        
    def test_follow(self):
        self.assertNotEqual((self.camera1.x, self.camera1.y), self.camera1.follow(self.player1, self.map1))
        
class PlayerTestCase(unittest.TestCase):
    def setUp(self):
        self.game2 = Game()
        self.game2.player = Player(self.game2, self.game2.all_sprites)
        self.game2.init_cam()
        self.vector1 = self.game2.player.point_at(100, 100)
        self.game2.player.rect.move_ip(0,1000)
        self.game2.player.bullets_num = 0
        
    def test_point_at(self):
        self.assertNotEqual(self.vector1, self.game2.player.point_at(100, 100))

    def test_reload(self):
        self.assertTrue(self.game2.player.reload() > 0)

class BonusTestCase(unittest.TestCase):
    def setUp(self):
        self.game3 = Game()
        self.game3.player = Player(self.game3, self.game3.all_sprites)

    def test_bonus_type1(self):
        self.assertEqual(self.game3.bonus.bonus_type(1)[0], 4)

    def test_bonus_type2(self):
        self.assertEqual(self.game3.bonus.bonus_type(2)[1], 25)

    def test_bonus_type3(self):
        self.assertEqual(self.game3.bonus.bonus_type(3)[2], 10)

    def test_bonus_type4(self):
        self.assertEqual(self.game3.bonus.bonus_type(4)[3], 6)
    def test_bonus_type5(self):
        self.assertEqual(self.game3.bonus.bonus_type(7)[4], 2)
    def test_bonus_type7(self):
        self.assertEqual(self.game3.bonus.bonus_type(5)[5], 0.6666666666666666)

class MobTestCase(unittest.TestCase):
    def setUp(self):
        self.game4 = Game()
        self.game4.player = Player(self.game4, self.game4.all_sprites)
        self.mob2 = Mob(250, 250, self.game4)
        self.game4.mobs.add(self.mob2)
        #print(self.mob2.path)
        
    def test_init(self):
        self.assertEqual(self.mob2.path[-1], (6, 11)) # [6, 11] is initial player position

class BulletTestCase(unittest.TestCase):
    def setUp(self):
        self.game5 = Game()
        #self.game5.player = Player(self.game5, self.game5.all_sprites)
        #self.game5.draw_map()
        self.house_1 = House(410, 10, "1")
        self.game5.bricks.add(self.house_1)
        self.water_1 = Water(20, 330, "W")
        self.game5.bricks.add(self.house_1)
        self.bullet1 = Bullet(10, 10, 40, 0, 10, 1, self.game5)
        self.bullet2 = Bullet(30, 30, 0, 30, 10, 2, self.game5)

    def test_update1(self):
        self.assertEqual(self.bullet1.update(), None)

    def test_update2(self):
        self.assertEqual(self.bullet2.update(), None)
